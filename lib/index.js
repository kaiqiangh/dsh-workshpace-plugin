...
var MemoryStore = class {
	...
	async open() {
		...
		for (let index = 0; index < lines.length; index += 1) {
			...
			if (!line) continue;
			try {
				const record = validateRecord(JSON.parse(line), this.scope, this.scopeKey, this.maxContentBytes);
	...
	list(options = {}) {
		...
		return Object.freeze([...this.records.values()].filter((record) => (options.type === void 0 || record.type === options.type) && (options.status === void 0 ? record.status === "active" : record.status === options.status)).sort((left, right) => right.updatedAt - left.updatedAt || left.id.localeCompare(right.id)).slice(0, limit));
	}
	async upsert(draft) {
		this.ensureWritable();
	...
	search(query, options = {}) {
		...
		const limit = safeLimit(options.limit, 100);
		return Object.freeze([...this.records.values()].filter((record) => record.status === (options.status ?? "active") && (options.type === void 0 || record.type === options.type)).filter((record) => recordRank(record, query) < 5).sort((left, right) => recordRank(left, query) - recordRank(right, query) || right.updatedAt - left.updatedAt || (right.lastUsedAt ?? 0) - (left.lastUsedAt ?? 0) || left.id.localeCompare(right.id)).slice(0, limit));
	}
	async compact() {
		this.ensureWritable();
...
var WorkspaceMemoryDomain = class {
	...
	async upsert(context, request, draft) {
		return (await this.store(context, request)).upsert(draft);
	}
...
//#endregion
export { MEMORY_MAX_CONTENT_BYTES, MEMORY_MAX_QUERY_BYTES, MEMORY_MAX_RESULTS, MEMORY_MAX_TAGS, MEMORY_MAX_TAG_BYTES, MEMORY_MAX_TITLE_BYTES, MEMORY_SCHEMA_VERSION, MemoryStore, MemoryStoreError, PreviewPanelError, PreviewService, WorkspaceArtifactCarrier, WorkspaceDeliverableError, WorkspaceMemoryDomain, WorkspaceService, apply, createPinnedContext, createWorkspaceDeliverable, deliverableResourceId, installWorkspaceResourceRoute, memoryStorePath, name, pinContextPath, registerPinnedContextCarrier, registerWorkspaceResourceRoute, safeDownloadName, sessionToolRecords, setContextCapacity, updateContextPath };

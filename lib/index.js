...
var WorkspaceArtifactCarrier = class {
	...
	async metadata() {
		...
		for (const item of deriveArtifacts(projection)) try {
...
* a disposer.
*/
function attachWorkspaceSummaryEmitter(ctx, memoryDomain) {
	const emitted = /* @__PURE__ */ new Set();
	const pending = /* @__PURE__ */ new Map();
	const timers = /* @__PURE__ */ new Map();
	const withMemoryCounts = async (agent, base) => {
		const cwd = agent.session?.header?.cwd;
		if (!cwd) return base;
		try {

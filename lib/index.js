...
function shellWritePaths(tool, args) {
	if (tool === void 0 || !/^(?:bash|sh|zsh|shell|terminal|exec)$/i.test(tool)) return [];
	...
	for (const match of command.matchAll(/(?:^|[\s;&|])\d*>>?\s*(?:"([^"]+)"|'([^']+)'|([^\s;&|]+))/gu)) add(match[1] ?? match[2] ?? match[3]);
	for (const match of command.matchAll(/(?:^|[;&|]\s*)(?:tee|touch)(?:\s+-[^\s]+)*\s+(?:"([^"]+)"|'([^']+)'|([^\s;&|]+))/gu)) add(match[1] ?? match[2] ?? match[3]);
	return paths;
}
function operationFromResult(tool, args, content) {

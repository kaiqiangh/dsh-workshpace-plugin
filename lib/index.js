...
function shellWordOnLine(command, start) {
	let index = start;
	while (index < command.length && /[ \t\r]/u.test(command[index] ?? "")) index += 1;
	return command[index] === "\n" ? void 0 : shellWordAt(command, index);
}
function shellHeredocEnd(command, start, delimiter, stripTabs = false) {
...
function shellRedirectionPaths(command, add) {
	...
	for (let index = 0; index < command.length; index += 1) {
		...
		if (character === "<" && command[index + 1] === "<" && command[index + 2] !== "<") {
			...
			if (delimiter !== void 0 && bodyStart >= 0) heredocSkipEnd = shellHeredocEnd(command, delimiterStart, delimiter, stripTabs);
			else index += 1;
			continue;
		}
		if (/[;&|\n]/u.test(character)) {
		...
		if (!segmentHeadChecked && !/\s/u.test(character)) {
			...
			if (/^(?:tee|touch)$/i.test(head ?? "")) {
				...
				if (head?.toLowerCase() === "touch") {
					if (target?.startsWith("-") && target !== "--") target = void 0;
					while (target !== void 0 && ![
						"#",
						"<",
						">"
					...
					while (target !== void 0 && ![
						...
						if (target.startsWith("-")) {
							if (![
								...
								"--append",
								"--ignore-interrupts"
							].includes(target) && !target.startsWith("--output-error=")) {
								optionsValid = false;
								break;
							}
						} else add(target);

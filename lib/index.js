...
var MemoryStore = class {
	...
	list(options = {}) {
		...
		return Object.freeze([...this.records.values()].filter((record) => (options.type === void 0 || record.type === options.type) && (options.status === void 0 ? record.status === "active" : record.status === options.status) && (record.governance?.expiresAt === void 0 || record.governance.expiresAt > this.now())).sort((left, right) => right.updatedAt - left.updatedAt || left.id.localeCompare(right.id)).slice(0, limit));
	}
	async upsert(draft) {
		this.ensureWritable();
	...
	search(query, options = {}) {
		...
		const limit = safeLimit(options.limit, 100);
		return Object.freeze([...this.records.values()].filter((record) => record.status === (options.status ?? "active") && (options.type === void 0 || record.type === options.type) && (record.governance?.expiresAt === void 0 || record.governance.expiresAt > this.now())).filter((record) => recordRank(record, query) < 5).sort((left, right) => recordRank(left, query) - recordRank(right, query) || right.updatedAt - left.updatedAt || (right.lastUsedAt ?? 0) - (left.lastUsedAt ?? 0) || left.id.localeCompare(right.id)).slice(0, limit));
	}
	async compact() {
		this.ensureWritable();
...
//#endregion
//#region packages/plugin/src/domain/memory-governance.ts
...
const retentionFor = (scope) => scope === "session" ? "session-end" : scope === "user" ? "user-managed" : "project-delete";
	...
	return parsed.records.map((record) => {
		...
		return Object.freeze({
			...
			governance: {
				...governance,
				origin: "imported",
				sourceRefs: [sourceRef("import", record.id)],
...
var WorkspaceMemoryDomain = class {
	...
	async upsert(context, request, draft) {
		...
		const previous = draft.id === void 0 ? void 0 : [
			"active",
			"archived",
			"forgotten"
		].flatMap((status) => store.list({
			status,
			limit: 100
		...
		const governance = draft.id === void 0 && store.list({
			status: "active",
			limit: 100
		}).find((record) => record.title.trim().toLocaleLowerCase() === draft.title.trim().toLocaleLowerCase()) ? {
			origin: "user-authored",
			sourceRefs: [],
			verification: "unverified",
			revision: 1,
			conflictGroup: conflictGroupFor(draft.title),
			retention: request.scope === "session" ? "session-end" : request.scope === "user" ? "user-managed" : "project-delete"
		} : draft.governance;

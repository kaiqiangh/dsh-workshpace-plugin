...
async function boundedDiff(root, args) {
	let output = await runGit(root, args);
	let truncated = false;
	if (Buffer.byteLength(output, "utf8") > 524288) {
...
function operationFromResult(tool, content) {
	if (tool === void 0 || !/^(?:write|write[-_]file|file[-_]write|create[-_]file)$/i.test(tool)) return void 0;
	const textParts = [];
...
//#endregion
//#region packages/plugin/src/host/workspace-summary.ts
...
*/
function attachWorkspaceSummaryEmitter(ctx) {
	...
	const flush = (agent) => {
		...
		agent.session?.append?.("workspace/summary", {
			id: agent.id,
			phase,
			summary
		});
...
let WorkspaceService = (() => {
	...
	return class WorkspaceService extends _classSuper {
		static {
			...
			_contextSnapshot_decorators = [Remote("contextSnapshot")];
			_replaceContext_decorators = [Remote("replaceContext")];
		...
		contextSnapshot(agentId) {
			return this.snapshot;
		}
		replaceContext(agentId, snapshot) {
		...
		async carrier(agent) {
			...
			try {
				...
				this.artifactCarrier = new WorkspaceArtifactCarrier({
					workspace,
					root,
					records: () => sessionToolRecords(agentView.session?.events ?? [])
				});
...
//#endregion
export { GIT_MAX_DIFF_BYTES, GitError, MEMORY_MAX_CONTENT_BYTES, MEMORY_MAX_FILE_BYTES, MEMORY_MAX_QUERY_BYTES, MEMORY_MAX_RESULTS, MEMORY_MAX_TAGS, MEMORY_MAX_TAG_BYTES, MEMORY_MAX_TITLE_BYTES, MEMORY_PROPOSE_SECTION, MEMORY_PROPOSE_TOOL_NAME, MEMORY_SCHEMA_VERSION, MEMORY_TYPES, MemoryGovernanceError, MemoryStore, MemoryStoreError, PreviewPanelError, PreviewService, WorkspaceArtifactCarrier, WorkspaceDeliverableError, WorkspaceMemoryDomain, WorkspaceService, apply, assertMemoryRevision, attachWorkspaceSummaryEmitter, conflictGroupFor, createMemoryProposeTool, createPinnedContext, createWorkspaceDeliverable, deliverableResourceId, exportMemoryBundle, gitDiff, gitStatus, importMemoryBundle, inject, installWorkspaceResourceRoute, isGitRepository, memoryGovernance, memoryGovernanceEligible, memoryStorePath, name, parsePorcelain, pinContextPath, proposeMemory, registerMemoryPropose, registerPinnedContextCarrier, registerWorkspaceResourceRoute, safeDownloadName, sessionToolRecords, setContextCapacity, sourceRef, transitionMemoryGovernance, updateContextPath, workspaceMemoryContextFor, workspaceSummaryFor };

...
async function gitHistory(root, options = {}) {
	...
	const output = await runGit(root, [
		"log",
		...options.scope === "localBranches" ? ["--branches", "--topo-order"] : [],
		"--max-count",
		String(limit),
		"--skip",
		String(offset),
...
function deriveArtifacts(projection) {
	return [...projection.files.values()].filter((file) => file.createdInSession && file.current === "present" && (file.attribution === "agent-evidenced" || file.attribution === "session-observed")).map((file) => ({
		path: file.path,

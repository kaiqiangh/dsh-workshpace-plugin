...
*/
async function proposeMemory(memoryDomain, agent, args) {
	assertProposalArgs(args);
	const cwd = agent.session?.header?.cwd;
	if (typeof agent.id !== "string" || !agent.id.trim() || !cwd) throw new MemoryStoreError("PROJECT_UNAVAILABLE", "Memory proposals require an active session");
	let snapshot;
	try {
...
function createMemoryProposeTool(memoryDomain) {
	return defineTool({
		...
		async execute(args, exec) {
			...
			const saved = await proposeMemory(memoryDomain, agent, args);
...
function registerMemoryPropose(ctx) {
...
//#endregion
export { MEMORY_MAX_CONTENT_BYTES, MEMORY_MAX_FILE_BYTES, MEMORY_MAX_QUERY_BYTES, MEMORY_MAX_RESULTS, MEMORY_MAX_TAGS, MEMORY_MAX_TAG_BYTES, MEMORY_MAX_TITLE_BYTES, MEMORY_PROPOSE_SECTION, MEMORY_PROPOSE_TOOL_NAME, MEMORY_SCHEMA_VERSION, MemoryGovernanceError, MemoryStore, MemoryStoreError, PreviewPanelError, PreviewService, WorkspaceArtifactCarrier, WorkspaceDeliverableError, WorkspaceMemoryDomain, WorkspaceService, apply, assertMemoryRevision, conflictGroupFor, createMemoryProposeTool, createPinnedContext, createWorkspaceDeliverable, deliverableResourceId, exportMemoryBundle, importMemoryBundle, inject, installWorkspaceResourceRoute, memoryGovernance, memoryGovernanceEligible, memoryStorePath, name, pinContextPath, proposeMemory, registerMemoryPropose, registerPinnedContextCarrier, registerWorkspaceResourceRoute, safeDownloadName, sessionToolRecords, setContextCapacity, sourceRef, transitionMemoryGovernance, updateContextPath };

...
test("renders one Workspace conversation view with artifact and memory tabs", () => {
  ...
  assert.equal(children[2].props["data-dsh-workspace"], "panel-tabs");

  const content = children[3].props.children as readonly { readonly props: Record<string, any> }[];
  assert.equal(content[0].props["data-dsh-workspace-tab"], "artifacts");
  assert.equal(content[1].props["data-dsh-workspace-tab"], "memory");
});

test("passes props through to both surfaces", () => {
  const render = createWorkspaceConversationViewComponent({
    artifacts: () => ({ type: "artifacts" }),
    memory: () => ({ type: "memory" }),
  });
